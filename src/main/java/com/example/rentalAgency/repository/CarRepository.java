...
//    @Query(value = "select c.car_name,c.car_brand, b.branch_name from Car c inner join Branch b on c.branch_id=b.id  ",nativeQuery = true)
    @Query("select new com.example.rentalAgency.dto.CarDTO(c.brand,c.model,c.branch.branchName,c.gearbox,c.imageUrl,c.feePerDay,c.mileage) from Car c inner join Branch b on c.branch.id=b.id")
    Iterable<CarDTO> findCarsDto();
